...
class Help(commands.Cog):
    def __init__(self, client):
        ...
        self.help_texts = json.loads(open(self.help_texts_path + 'help.json', 'r').read())
...
To bring up this message again, type `{ctx.prefix}help`!""", embed=embed)
            ...
            await author.send(content=f"""To bring up this message again, type `{ctx.prefix}help {command}`!""", embed=embed)
