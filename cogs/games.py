...
{self.rps_emotes[move]}      {self.rps_emotes[bot_move]}""")
    ...
    async def guessinggame(self, ctx, highest=100):
        ...
        if highest in range(2, 1_000_001):
            if highest >= 100_000:
                await ctx.send(f"""{mention} A game with this max will take a while! Are you sure you want to do this?
Type `yes` to confirm or anything else to cancel.""")
        ...
        # Starting prompt
        await ctx.send(f"""{mention} I'm thinking of a number between 1 and {highest}.
        ...
        while play:
            ...
            if guess.content == "cancel":
                ...
                if guess not in range(1, highest + 1):
                    await ctx.send(f"{mention} That's not a number between 1 and {highest}!")
                else:
                    guesses += 1
                    if guess > number:
                        ...
                        if guesses > 1:
                            await ctx.send(f"{mention} You got it! It took you {guesses} tries.")
                        else:
                            await ctx.send(f"{mention} Unbelievable! You got it on your first try!!")
                        play = False
